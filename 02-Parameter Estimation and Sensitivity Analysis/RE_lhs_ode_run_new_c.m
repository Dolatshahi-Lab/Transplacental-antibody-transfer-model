function CalibrationOutput = RE_lhs_ode_run_new_c(odeSettings, runIteration)
...
% These binding parameters do not change - RE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
param.kon1 = 5.4e6; param.koff1 = 0.0675;
param.kon2 = 3.375e6; param.koff2 = 0.0675;
param.kon3 = 2.025e6; param.koff3 = 0.0675;
param.kon4 = 1.35e6; param.koff4 = 0.0675;
param.kon1b = 5.4e6; param.koff1b = 54;
param.kon2b = 1.08e6; param.koff2b = 54;
param.kon3b = 1.08e7; param.koff3b = 54;
param.kon4b = 1.08e7; param.koff4b = 54;
param.d_ab = log(2)/31; %antibody decay rate
...
for run = 1:odeSettings.NR
    ...
    try
        if ~isfield(odeSettings, 'pulseTimePoints')
            ...
            % RE addition:  define parameters outside of the run
            param.k_up = runParameters(1);
            param.k_t = runParameters(2);
            param.v_m = runParameters(5);
            param.v_f = runParameters(6);
            param.v_str = runParameters(7);
            param.v_stb = runParameters(8);
            param.v_ec = runParameters(9);
            param.k_deg = runParameters(10);
%             param.d_ab = log(2)/31*param.v_f; %antibody decay rate

            % solve Fc receptor trend
            ratday = [1 15 21]'*40/21; %rat days scaled to human weeks GA
            fcrn = [0 runParameters(3)/2 runParameters(3)]'; param.fcrn_curve = fit(ratday,fcrn,'poly2');
            fcgr2b = [0 runParameters(4)/2 runParameters(4)]'; param.fcgr2b_curve = fit(ratday,fcgr2b,'poly2');
            
            lastwarn('') % Clear last warning message
            [t,y] = ode15s(@(t,y)odeSettings.odeModelHandle(t, y, param), odeSettings.tspan, y0, options);
